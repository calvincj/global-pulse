...
export const BRI        = new Set(['CHN','PAK','KAZ','UZB','TKM','TJK','KGZ','LAO','KHM','MMR','LKA','BGD','NPL','MNG','IDN','MYS','PHL','EGY','SDN','ETH','KEN','NGA','AGO','ZMB','DJI','TZA','GHA','COD','MAR','DZA','TUN','IRQ','IRN','SAU','ARE','QAT','KWT','OMN','BHR','JOR','LBN','SRB','HUN','GRC','BLR','ARG','BOL','CHL','ECU','PER','VEN','URY','CUB','PNG','FJI','WSM','TON','SLB','VUT','NZL']); // Belt and Road MOU signatories (curated; Italy withdrew 2023)
export const OPEC_PLUS   = new Set(['SAU','IRQ','IRN','ARE','KWT','DZA','NGA','COG','GNQ','GAB','VEN','LBY','RUS','KAZ','AZE','OMN','BHR','MYS','BRN','SDN','SSD','MEX']);
export const SADC        = new Set(['ZAF','AGO','BWA','COM','COD','SWZ','LSO','MDG','MWI','MUS','MOZ','NAM','SYC','TZA','ZMB','ZWE']);
export const ECOWAS      = new Set(['BEN','CPV','CIV','GMB','GHA','GIN','GNB','LBR','NGA','SEN','SLE','TGO']); // Mali, Niger, Burkina Faso withdrew Jan 2025 (Alliance of Sahel States)
export const V4          = new Set(['POL','CZE','SVK','HUN']);
export const PACIFIC_ALLIANCE = new Set(['MEX','COL','CHL','PER']);
export const ALBA        = new Set(['CUB','VEN','BOL','NIC']);
export const CPTPP       = new Set(['JPN','CAN','AUS','NZL','SGP','VNM','MYS','BRN','MEX','PER','CHL','GBR']);
...
export const BLOCS: Record<string, { name: string; members: Set<string> }> = {
  ...
  BRI:        { name: 'Belt and Road Initiative', members: BRI },
  OPEC_PLUS:  { name: 'OPEC+',            members: OPEC_PLUS },
  SADC:       { name: 'SADC',             members: SADC },
  ECOWAS:     { name: 'ECOWAS',           members: ECOWAS },
  V4:         { name: 'Visegrad Group',   members: V4 },
  PACIFIC_ALLIANCE: { name: 'Pacific Alliance', members: PACIFIC_ALLIANCE },
  ALBA:       { name: 'ALBA',             members: ALBA },
  CPTPP:      { name: 'CPTPP',            members: CPTPP },
};

export function getBlocMemberships(iso3: string): { key: string; name: string }[] {
...
function blocScore(a: string, b: string): { polScore: number; ecoScore: number; polNotes: string; ecoNotes: string } | null {
  ...
  const opec   = OPEC_PLUS.has(a) && OPEC_PLUS.has(b);
  const sadc   = SADC.has(a) && SADC.has(b);
  const ecowas = ECOWAS.has(a) && ECOWAS.has(b);
  const v4     = V4.has(a) && V4.has(b);
  const pacAll = PACIFIC_ALLIANCE.has(a) && PACIFIC_ALLIANCE.has(b);
  const alba   = ALBA.has(a) && ALBA.has(b);
  const cptpp  = CPTPP.has(a) && CPTPP.has(b);
  const bri    = BRI.has(a) && BRI.has(b) && (a === 'CHN' || b === 'CHN');
  ...
  if (opec)   return { polScore: 0.35, ecoScore: 0.55, polNotes: 'OPEC+ oil-exporter coalition; coordinates production policy despite sharp geopolitical divergence between members', ecoNotes: 'Joint production quotas shape global oil prices and market share; deep disparity in wealth and alignment among signatories' };
  if (sadc)   return { polScore: 0.35, ecoScore: 0.35, polNotes: 'Southern African Development Community; regional security coordination and peacekeeping missions', ecoNotes: 'SADC Free Trade Protocol; cross-border labor migration and shared Southern African power grid' };
  if (ecowas) return { polScore: 0.38, ecoScore: 0.38, polNotes: 'ECOWAS regional bloc; historically intervened against coups, though Sahel states withdrew in 2025', ecoNotes: 'ECOWAS common external tariff and free movement protocol; West African CFA franc monetary overlap' };
  if (v4)     return { polScore: 0.55, ecoScore: 0.62, polNotes: 'Visegrad Group; Central European policy coordination, increasingly split over Russia and rule-of-law disputes', ecoNotes: 'Deep Central European manufacturing supply chains and cross-border labor markets' };
  if (pacAll) return { polScore: 0.55, ecoScore: 0.68, polNotes: 'Pacific Alliance; market-oriented Latin American bloc aligned on trade liberalization and democratic governance', ecoNotes: 'Pacific Alliance free trade area; integrated MILA stock exchange and near-total tariff elimination' };
  if (alba)   return { polScore: 0.55, ecoScore: 0.30, polNotes: 'ALBA Bolivarian Alliance; leftist anti-US regional solidarity pact', ecoNotes: 'Oil-financing and barter arrangements in the Petrocaribe mold; limited formal trade integration' };
  if (cptpp)  return { polScore: 0.40, ecoScore: 0.68, polNotes: 'CPTPP trans-Pacific trade partnership; rules-based order signaling amid US absence and pending Chinese application', ecoNotes: 'CPTPP eliminates tariffs on 95%+ of goods among members; one of the world\'s largest free trade areas by GDP' };
  if (bri)    return { polScore: 0.30, ecoScore: 0.55, polNotes: 'Belt and Road Initiative partner; signed Chinese infrastructure cooperation MOU, deepening diplomatic alignment with Beijing', ecoNotes: 'Chinese state-backed loans and construction for ports, rail, and energy infrastructure; debt sustainability concerns in several partner states' };
  return null;
}
...
export const RELATIONSHIPS: Relationship[] = [
  ...
  rel('ARE','PAK', 0.58, 0.45, 8,    "Strong ties; large Pakistani diaspora; UAE investment in Pakistan; UAE mediates Pakistan's IMF deals"),
  // ── TURKEY ──────────────────────────────────────────────────────────────
  ...
  rel('ISR','SAU', 0.10, 0.08, 1,    "No formal diplomatic relations; pre-Oct 7 normalization track (Abraham Accords expansion) stalled; MBS ties any deal to a credible Palestinian statehood path; quiet security coordination vs Iran continues"),
  rel('ISR','BHR', 0.28, 0.20, 1,    "Abraham Accords 2020; formal ties but Manama recalled its ambassador and suspended economic relations Nov 2023 over Gaza war; hosts US 5th Fleet"),
  rel('ISR','MAR', 0.42, 0.35, 1,    "Abraham Accords 2020; normalization linked to US recognition of Moroccan sovereignty over Western Sahara; growing defense-industrial and drone cooperation; large Moroccan-Jewish diaspora ties"),
  rel('ISR','SDN', 0.05, 0.02, 0,    "Agreed to normalize under Abraham Accords Oct 2020; never ratified; 2021 coup and 2023 civil war between SAF and RSF left normalization dormant"),
  rel('ISR','KWT',-0.10,-0.02, 0,    "No diplomatic relations; Kuwaiti law criminalizes normalization and bars Israeli passport holders; parliament repeatedly reaffirmed anti-normalization stance"),
  rel('ISR','QAT',-0.05, 0.05, 0,    "No formal relations; Doha hosts Hamas's political bureau since 2012 (at US request as a negotiating channel); mediates hostage/ceasefire talks alongside Egypt and the US; funds Gaza reconstruction"),
  rel('ISR','IRQ',-0.35,-0.15, 0,    "No diplomatic relations; Iraq's 2022 law criminalizes normalization (life imprisonment or death); Iran-aligned PMF militias have launched drone and rocket attacks on Israel"),
  rel('ISR','YEM',-0.60,-0.40, 0,    "Houthi rebels launched missile and drone attacks on Israel and blockaded Red Sea shipping since Oct 2023; Israel struck Hodeidah port and Sanaa in retaliation 2024; no relations"),
  rel('ISR','LBY',-0.10,-0.05, 0,    "No diplomatic relations; Arab League boycott; 2023 informal meeting between Libyan and Israeli foreign ministers in Rome triggered domestic riots and the Libyan FM's flight from the country"),
  rel('ISR','OMN', 0.10, 0.08, 0,    "No formal diplomatic relations but long history of discreet engagement; hosted Rabin 1994 and briefly reopened a trade office in the 1990s; serves as a back-channel between Israel, Iran, and the US"),
  rel('ISR','CYP', 0.55, 0.45, 2,    "Close strategic partnership; East Mediterranean Gas Forum; EastMed pipeline discussions; joint naval exercises; evacuation hub for Israelis during the 2024 Lebanon war"),
  rel('ISR','GRC', 0.58, 0.42, 2,    "Israel-Greece-Cyprus trilateral; joint Iniochos air force exercises; Israeli air-defense sales to Greece; EastMed energy cooperation"),
  // ── EGYPT ───────────────────────────────────────────────────────────────
...
export function getRelationshipsFor(countryId: string): Relationship[] {
  ...
  const BLOC_ORDER = [NATO, EU, FIVE_EYES, AUKUS, QUAD, G7, CSTO, SCO, BRICS, GCC, ASEAN, MERCOSUR, ARAB_LEAGUE, AU, BRI, OPEC_PLUS, SADC, ECOWAS, V4, PACIFIC_ALLIANCE, ALBA, CPTPP];
